...
class PolarMaker:
    ...
    def set_epoch(self, file_name, epoch_idx):
        ...
        # Speed circles
        speed_step = 2
        for speed in range(self.max_speed, self.min_speed, -speed_step):
            ulx, uly = self.to_screen((-speed, speed))
            lrx, lry = self.to_screen((speed, -speed))
            draw.arc((ulx, uly, lrx, lry), start=180, end=0, fill=NON_TRANSPARENT_WHITE_FONT_COLOR)
        ...
        for angle in range(-90, 100, 30):
            ...
            draw.line((x1, y1, x2, y2), fill=NON_TRANSPARENT_WHITE_FONT_COLOR)
        ...
        for idx in range(epoch_idx):
            ...
            color = '#{r:02X}{g:02X}{b:02X}{a:02X}'.format(r=255, g=0, b=0, a=a)
            if idx == epoch_idx - 1:
                color = '#{r:02X}{g:02X}{b:02X}{a:02X}'.format(r=255, g=0, b=0, a=a)
                draw.line((x1, y1, x2, y2), fill=NON_TRANSPARENT_WHITE_FONT_COLOR, width=3)
                draw.ellipse((ulx, uly, lrx, lry), fill=color, outline=(0, 0, 0), width=4)
            else:
                draw.ellipse((ulx, uly, lrx, lry), fill=color, outline=(0, 0, 0))
...
if __name__ == '__main__':
    polar_maker = PolarMaker('/tmp', 'polar_test', 800, None, True)

    polar_maker.set_history('tack', fake_epochs())
    polar_maker.target = (7, 35)
    polar_maker.set_epoch('polar.png', 60)
