...
START_TEST(test_minmea_parse_vwr1)
{
    const char *sentence = "$IIVWR,023,R,08.8,N,04.5,M,016.2,K*52";
    struct minmea_sentence_vwr frame = {};
    static const struct minmea_sentence_vwr expected = {
		.twa_valid = true,
		.twa_deg = { 23, 1 },
		.tws_valid = true,
		.tws_kts = { 88, 10 },
    ...
    bool ok = minmea_parse_vwr(&frame, sentence);
    ck_assert(ok  == true);
    ck_assert(!memcmp(&frame, &expected, sizeof(frame)));
}
END_TEST

START_TEST(test_minmea_parse_lsn_sps_rdy)
{
    const char *sentence = "$POTOR,LSN,SPS,RDY";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_SPS,
		.u.sps.state = SPS_STATE_READY,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
    bool ok = minmea_parse_lsn(&frame, sentence);
    ck_assert(ok  == true);
    ck_assert(!memcmp(&frame, &expected, sizeof(frame)));
}
END_TEST

START_TEST(test_minmea_parse_lsn_sps_str)
{
    const char *sentence = "$POTOR,LSN,SPS,STR";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_SPS,
		.u.sps.state = SPS_STATE_START,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
    bool ok = minmea_parse_lsn(&frame, sentence);
...
START_TEST(test_minmea_parse_lsn_sps_prc)
{
    const char *sentence = "$POTOR,LSN,SPS,PRC";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_SPS,
		.u.sps.state = SPS_STATE_PROCESSING,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
    bool ok = minmea_parse_lsn(&frame, sentence);
...
START_TEST(test_minmea_parse_lsn_trn)
{
    const char *sentence = "$POTOR,LSN,TRN,UP,5";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_TRN,
		.u.trn.direction = TURN_UP,
		.u.trn.deg = 5,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
...
START_TEST(test_minmea_parse_lsn_tck)
{
    const char *sentence = "$POTOR,LSN,TCK,RDY";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_TCK,
		.u.tck.cmd = TACK_OR_JIBE_READY,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
    bool ok = minmea_parse_lsn(&frame, sentence);
...
START_TEST(test_minmea_parse_lsn_jbe)
{
    const char *sentence = "$POTOR,LSN,JBE,NOW";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_JBE,
		.u.jbe.cmd = TACK_OR_JIBE_NOW,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
    bool ok = minmea_parse_lsn(&frame, sentence);
...
START_TEST(test_minmea_parse_lsn_ctl)
{
    const char *sentence = "$POTOR,LSN,CTL,OFF";
    struct minmea_sentence_lsn frame = {};
    static const struct minmea_sentence_lsn expected = {
		.msg = MINMEA_POTOR_LSN_CTL,
		.u.ctl.owner = HUMAN,
    };
    ck_assert(minmea_check(sentence, false) == true);
    ck_assert(minmea_check(sentence, true) == false);
...
static Suite *minmea_suite(void)
{
    ...
    //tcase_add_test(tc_parse, test_minmea_parse_gll1);  Fails for some reason
    ...
    tcase_add_test(tc_parse, test_minmea_parse_vwr1);

    tcase_add_test(tc_parse, test_minmea_parse_lsn_sps_prc);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_sps_rdy);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_sps_str);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_trn);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_tck);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_jbe);
    tcase_add_test(tc_parse, test_minmea_parse_lsn_ctl);
