...
bool minmea_parse_vwr(struct minmea_sentence_vwr *frame, const char *sentence)
{
	// $IIVWR,023,R,08.8,N,04.5,M,016.2,K*52
    char type[6];
    char direction;
    char unit;
    struct minmea_float tws_mps;
    struct minmea_float tws_kph;

    if (!minmea_scan(sentence, "tfcfcfcfc",
            type,
            &frame->twa_deg,&direction,
            &frame->tws_kts,&unit,
			&tws_mps,&unit,
			&tws_kph,&unit))
        return false;
    ...
    frame->twa_valid = frame->twa_deg.value > -1;
    frame->tws_valid = frame->tws_kts.value > -1;

    if ( direction != 'R')
    	frame->twa_deg.value *= -1;
...
bool minmea_parse_lsn(struct minmea_sentence_lsn *frame, const char *sentence)
{
    char type[6];
    char subtype[6];
    char cmd[6];
    ...
    if (!minmea_scan(sentence, "tss;si",
            type,
            subtype,
            cmd,
			arg1,
    ...
    if (strcmp(subtype, "LSN"))
        return false;

    if ( !strcmp( cmd,"SPS" )){
    	frame->msg = MINMEA_POTOR_LSN_SPS;
    	if ( !strcmp( arg1,"RDY" ))
    		frame->u.sps.state = SPS_STATE_READY;
    	else if ( !strcmp( arg1,"STR" ))
    		frame->u.sps.state = SPS_STATE_START;
    	else if ( !strcmp( arg1,"PRC" ))
    		frame->u.sps.state = SPS_STATE_PROCESSING;
    	else
    		return false;
    }else if ( !strcmp( cmd,"TRN" )){
    	frame->msg = MINMEA_POTOR_LSN_TRN;
    	if ( !strcmp( arg1,"UP" ))
    		frame->u.trn.direction = TURN_UP;
    	else if ( !strcmp( arg1,"DOWN" ))
    		frame->u.trn.direction = TURN_DOWN;
    	else if ( !strcmp( arg1,"LEFT" ))
    		frame->u.trn.direction = TURN_LEFT;
    	else if ( !strcmp( arg1,"RIGHT" ))
    		frame->u.trn.direction = TURN_RIGHT;
    	else
    		return false;
    	frame->u.trn.deg = deg;
    }else if ( !strcmp( cmd,"TCK" )){
    	frame->msg = MINMEA_POTOR_LSN_TCK;
    	if ( !strcmp( arg1,"RDY" ))
    		frame->u.tck.cmd = TACK_OR_JIBE_READY;
    	else if ( !strcmp( arg1,"NOW" ))
    		frame->u.tck.cmd = TACK_OR_JIBE_NOW;
    	else
    		return false;
    }else if ( !strcmp( cmd,"JBE" )){
    	frame->msg = MINMEA_POTOR_LSN_JBE;
    	if ( !strcmp( arg1,"RDY" ))
    		frame->u.jbe.cmd = TACK_OR_JIBE_READY;
    	else if ( !strcmp( arg1,"NOW" ))
    		frame->u.jbe.cmd = TACK_OR_JIBE_NOW;
    	else
    		return false;
    }else if ( !strcmp( cmd,"CTL" )){
    	frame->msg = MINMEA_POTOR_LSN_CTL;
    	if ( !strcmp( arg1,"ON" ))
    		frame->u.ctl.owner= AUTOPLIOT;
    	else if ( !strcmp( arg1,"OFF" ))
    		frame->u.ctl.owner = HUMAN;
    	else
    		return false;
    }else{
    	return false;
